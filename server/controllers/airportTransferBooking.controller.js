...
exports.getBookings = async (req, res, next) => {
  try {
    ...
    query = AirportTransferBooking.find(JSON.parse(queryStr))
      .populate('transfer', 'airportName airportCode oneWayPrice roundTripPrice vehicleType')
    ...
    const total = await AirportTransferBooking.countDocuments(JSON.parse(queryStr));
...
exports.createBooking = async (req, res, next) => {
  try {
    ...
    if (req.body.tripType === 'one-way') {
      totalPrice = transfer.oneWayPrice * req.body.passengers;
    } else {
      totalPrice = transfer.roundTripPrice * req.body.passengers;
    }
...
exports.updateBooking = async (req, res, next) => {
  try {
    ...
    if (req.body.transfer && req.body.transfer !== booking.transfer.toString()) {
      const transfer = await AirportTransfer.findById(req.body.transfer);
    ...
    booking = await AirportTransferBooking.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true
    }).populate('transfer', 'airportName airportCode oneWayPrice roundTripPrice vehicleType')
      .populate('user', 'name email');
    ...
    const stats = await AirportTransferBooking.aggregate([
      {
        $facet: {
          ...
          guestBookings: [{ $match: { isGuestBooking: true } }, { $count: "count" }],
          userBookings: [{ $match: { isGuestBooking: false } }, { $count: "count" }],
